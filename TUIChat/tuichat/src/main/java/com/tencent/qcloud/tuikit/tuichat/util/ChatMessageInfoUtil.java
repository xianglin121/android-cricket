...
//        }
    ...
    private static MessageInfo createCustomMessageInfo(V2TIMMessage timMessage, Context context) {
        ...
        if (customJsonMap.get("type") != null) {
            int type = new BigDecimal(customJsonMap.get("type").toString()).intValue();
            if (MessageInfo.MSG_TYPE_GIFT == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_GIFT);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_COLOR_DANMU == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_COLOR_DANMU);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_NOBEL_ENTER == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_NOBEL_ENTER);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_BG_DANMU == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_BG_DANMU);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_BG_DANMU == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_BG_DANMU);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_RED_ENVELOPE == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_RED_ENVELOPE);
                msgInfo.setExtra(data);
            }else if (MessageInfo.MSG_TYPE_BROADCAST == type) {
                msgInfo.setMsgType(MessageInfo.MSG_TYPE_BROADCAST);
                msgInfo.setExtra(data);
            }
        }else {
            // 欢迎消息
