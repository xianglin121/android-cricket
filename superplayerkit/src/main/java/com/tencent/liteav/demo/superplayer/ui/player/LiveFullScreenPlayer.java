...
import com.tencent.liteav.demo.superplayer.ui.view.FullScreenDanmuSettingView;
import com.tencent.liteav.demo.superplayer.ui.view.InputDanmuDialogFragment;
 ...
 */
public class LiveFullScreenPlayer extends AbsPlayer implements View.OnClickListener,
    ...
    private TextView                       mTvVodQuality;                          // 画质文本
    ...
    private FullScreenDanmuSettingView     mVodMoreView;                           // 更多设置弹窗
    private VodQualityFullScreenView       mQualityView;                           // 画质设置弹窗
    private ImageView                      mWindowView;                          // 窗口模式
    ...
    public LiveFullScreenPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize(context);
    }

    public LiveFullScreenPlayer(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                    mSeekBarProgress.setProgress(progress);
    ...
    private void initView(Context context) {
        ...
        LayoutInflater.from(context).inflate(R.layout.superplayer_vod_player_live_fullscreen, this);
...
//        mLayoutBottom = (LinearLayout) findViewById(R.id.superplayer_ll_bottom);
        ...
        mQualityView = findViewById(R.id.vod_quality);
        mVodMoreView.setCallBack(new FullScreenDanmuSettingView.CallBack() {
            @Override
            public void OnToggleDanmuPosition(int position) {
                if (mControllerCallback != null) {
                    mControllerCallback.onToggleDanmuPosition(position);
                }
            }
        ...
        mQualityView.setCallBack(new VodQualityFullScreenView.CallBack() {
            @Override
        ...
        mTvVodQuality.setOnClickListener(this);
        findViewById(R.id.tv_danmu_input).setOnClickListener(this);
    ...
    public void updateQuality(int type) {
        mQualityView.updateQuality(type);
        if (type == 0) {
            mTvVodQuality.setText(getContext().getString(R.string.HDR));
        } else if (type == 1) {
            mTvVodQuality.setText(getContext().getString(R.string.HD));
        } else if (type == 2) {
            mTvVodQuality.setText(getContext().getString(R.string.SD));
        } else if (type == 3) {
            mTvVodQuality.setText(getContext().getString(R.string.fluent));
        }
    }
...
//                mVodMoreView.updatePlayType(SuperPlayerDef.PlayerType.LIVE_SHIFT);
    ...
    public void onClick(View view) {
        ...
        if (i == R.id.superplayer_iv_back || i == R.id.superplayer_tv_title) { //顶部标题栏
        ...
        } else if (i == R.id.live_iv_pause) {            //暂停\播放按钮
            togglePlayState();
            ...
            showMoreView();
        } else if (i == R.id.superplayer_iv_window) {             //恢复窗口按钮
//            mControllerCallback.onClickFloatingMode();
            if (mControllerCallback != null) {
                mControllerCallback.onBackPressed(SuperPlayerDef.PlayerMode.FULLSCREEN);
            }
    ...
    private void showInputTextMsgDialog(int state) {
        ...
        inputDanmuDialogFragment.setArguments(bundle);
        inputDanmuDialogFragment.show(((AppCompatActivity)getContext()).getSupportFragmentManager(), "InputDanmuDialogFragment");
    }
...
//        }
    ...
    public void setBarrageToggle(boolean isOn) {
        mBarrageOn = isOn;
        if (mBarrageOn) {
            mIvDanmu.setImageResource(R.drawable.superplayer_ic_danmuku_on);
        } else {
    ...
    private static class HideLockViewRunnable implements Runnable {
        private WeakReference<LiveFullScreenPlayer> mWefControllerFullScreen;

        public HideLockViewRunnable(LiveFullScreenPlayer controller) {
