...
//        ppiv_cover.height = (int)(width * 0.5625 * 0.9);
        ...
        helper.setGone(R.id.iv_live_status,...);
        ...
        helper.setText(R.id.tv_time, TextUtils.isEmpty(item.addtime)?"":item.addtime);
    }
    ...
    protected void convertPayloads(@NonNull BaseViewHolder helper, GameHistoryBean item, @NonNull List<Object> payloads) {
        if (payloads != null && payloads.size() > 0) {
            ...
            if(iv_like!=null){
                iv_like.setSelected((item.isAttention == 0)?false:true);
            }
        }
    }
